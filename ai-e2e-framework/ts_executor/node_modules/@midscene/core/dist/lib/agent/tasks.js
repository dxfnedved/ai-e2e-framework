...
class TaskExecutor {
    ...
    async convertPlanToExecutable(plans, modelConfig) {
        const tasks = [];
    ...
    createPlanningTask(userInstruction, actionContext, modelConfig) {
        const task = {
            ...
            executor: async (param, executorContext)=>{
                ...
                const planResult = await (uiTarsModelVersion ? index_js_namespaceObject.uiTarsPlanning : external_index_js_namespaceObject.plan)(param.userInstruction, {
                    context: uiContext,
                    actionContext,
    ...
    async action(userPrompt, modelConfig, actionContext) {
    ...
    createTypeQueryTask(type, demand, modelConfig, opt, multimodalPrompt) {
        const queryTask = {
            ...
            executor: async (param, taskContext)=>{
                ...
                if (ifTypeRestricted && 'Assert' === type) {
    ...
    async assert(assertion, modelConfig, opt) {
        const { textPrompt, multimodalPrompt } = (0, external_utils_js_namespaceObject_1.parsePrompt)(assertion);
        return await this.createTypeQueryExecution('Assert', textPrompt, modelConfig, opt, multimodalPrompt);
    }
    ...
    async waitFor(assertion, opt, modelConfig) {
        ...
        while(Date.now() - overallStartTime < timeoutMs){
            startTime = Date.now();
            const queryTask = await this.createTypeQueryTask('Assert', textPrompt, modelConfig, {
