...
const groupAndCaseForTest = (testInfo)=>{
    ...
    return {
        file: taskFile,
        id: (0, utils_namespaceObject.replaceIllegalPathCharsAndSpace)(`${taskFile}(${taskTitle})`),
        title: (0, utils_namespaceObject.replaceIllegalPathCharsAndSpace)(taskTitleWithRetry)
    };
};
const midsceneAgentKeyId = '_midsceneAgentId';
const midsceneDumpAnnotationId = 'MIDSCENE_DUMP_ANNOTATION';
const PlaywrightAiFixture = (options)=>{
    const { forceSameTabNavigation = true, waitForNetworkIdleTimeout = constants_namespaceObject.DEFAULT_WAIT_FOR_NETWORK_IDLE_TIMEOUT, waitForNavigationTimeout = constants_namespaceObject.DEFAULT_WAIT_FOR_NAVIGATION_TIMEOUT } = options ?? {};
    const pageAgentMap = {};
    const createOrReuseAgentForPage = (page, testInfo, opts)=>{
        let idForPage = page[midsceneAgentKeyId];
        if (!idForPage) {
            idForPage = (0, utils_namespaceObject.uuid)();
    ...
    const updateDumpAnnotation = (test, dump)=>{
        const tempFileName = `midscene-dump-${test.testId || (0, utils_namespaceObject.uuid)()}-${Date.now()}.json`;
